  ...
  checkOut() {
    if (this.postForm.valid) {
      Swal.fire({
        ...
        if (result.isConfirmed) { // Chỉ tiếp tục nếu người dùng xác nhận
          let email = this.sessionService.getUser();
          this.cartService.getCart(email).subscribe(data => {
            this.cart = data as Cart;
            this.cart.address = this.postForm.value.number + ', ' + this.wards.find((item) => item.code == this.wardCode)?.name + ', ' + this.district.name + ', ' + this.province.name;
